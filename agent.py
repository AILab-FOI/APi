...
class APiAgent(APiBaseAgent):
    ...
    def __init__(self, agentname, name, password, holon, token, args=[], flows=[]):
        ...
        self.forward_channels = set(
            i for i in self.flows if i[0] != 'self' and i[1] != 'self')
        ...
        for srv in self.output_channel_servers.values():
            ...
            while not sent:
                try:
                    ...
                    print('ERROR SENDING', data, 'TO',
                          srv['server'], srv['port'], '(BROKEN PIPE)')
    ...
    def start_shell_stdin(self, prompt=False):
        ...
        self.shell_socket_stdin.setsockopt(
    ...
    def start_shell_stdout(self):
        ...
        self.shell_socket_stdout.setsockopt(
            socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    ...
    def start_shell_client(self, prompt=True, await_stdin=True, print_stdout=True, print_stderr=False):
        ...
        if not self.shell_ip and not self.shell_port:
            ...
            if await_stdin:
                self.dtach_session = os.path.join(
                    TMP_FOLDER, self.name, 'stdin')
            elif print_stdout:
                self.dtach_session = os.path.join(
                    TMP_FOLDER, self.name, 'stdout')
            elif print_stderr:
                self.dtach_session = os.path.join(
                    TMP_FOLDER, self.name, 'stderr')
            else:
                err = 'No input and not output specified!'
                raise APiShellInitError(err)
            cmd = 'dtach -A %s ./../APishc.py %s %d' % (
                self.dtach_session, self.shell_ip, self.shell_port)
            if await_stdin:
                cmd += ' --input'
            elif print_stdout:
                cmd += ' --output'
    ...
    def all_setup(self):
        ...
        try:
            connected_inputs = set(
                [ch for ch in self.input_channel_servers.keys()])
        ...
        async def run(self):
            ...
            for out in self.agent.output_channel_query_buffer:
                ...
                try:
                    ...
                    self.agent.say('Could not find channel', out,
                                   'in address book, querying')
                    metadata = self.agent.query_msg_template
                    metadata['reply-with'] = str(uuid4().hex)
    ...
    class SubscribeToInputChannels(OneShotBehaviour):
        async def run(self):
            ...
            self.agent.say('Subscribing to inputs:',
                           self.agent.input_channel_query_buffer)
            for inp in self.agent.input_channel_query_buffer:
    ...
    class AttachToOutputChannels(OneShotBehaviour):
        async def run(self):
            ...
            self.agent.say('Attaching to outputs',
                           self.agent.output_channel_query_buffer)
        ...
        async def run(self):
            ...
            if msg:
                if self.agent.verify(msg):
                    self.agent.say(
                        '(QueryChannels) Message verified, processing ...')
                    channel = msg.metadata['address']
                    try:
                        self.agent.input_ack.remove(
                        ...
                        self.agent.say(
                            'I have no memory of this message (%s). (awkward Gandalf look)' % msg.metadata['in-reply-to'])
                else:
                    self.agent.say(
                        'Message could not be verified. IMPOSTER!!!!!!')
                    ...
                    try:
                        ...
                        if msg.metadata['performative'] == 'refuse':
                            ...
                            if len(self.agent.output_channel_servers) == len(self.agent.output_channels) and len(self.agent.input_channel_servers) == len(self.agent.input_channels):
                                metadata = deepcopy(
                                    self.agent.inform_msg_template)
                ...
                if self.agent.verify(msg):
                    ...
                    try:
                        ...
                        if msg.metadata['performative'] == 'refuse':
                            ...
                            self.agent.say(
                                '(SetupOutputChannels) Setting up', msg.metadata['type'], 'channel', channel)
                            servers[channel] = {}
                            servers[channel]['server'] = msg.metadata['server']
                            servers[channel]['port'] = int(
                                msg.metadata['port'])
        ...
        async def run(self):
            ...
            if self.agent.all_setup():
                for srv in self.agent.input_channel_servers.values():
                    ...
                    if result:
                        self.agent.say('(Listen) Received', result,
                                       'from server', srv['server'], srv['port'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('name', metavar='NAME', type=str,
                        help="Agent's local APi name")
    parser.add_argument('address', metavar='ADDRESS',
                        type=str, help="Agent's XMPP/JID address")
    parser.add_argument('password', metavar='PWD', type=str,
                        help="Agent's XMPP/JID password")
    parser.add_argument('holon', metavar='HOLON', type=str,
                        help="Agent's instantiating holon's XMPP/JID address")
    parser.add_argument('token', metavar='TOKEN', type=str,
                        help="Agent's security token")
    parser.add_argument('args', metavar='ARGS', type=str,
                        help="Agent's instantiation arguments")
    parser.add_argument('flows', metavar='FLOWS', type=str,
                        help="Agent's communication flows")

    args = parser.parse_args()
    main(args.name, args.address, args.password,
         args.holon, args.token, args.args, args.flows)
